...
namespace RESTfulFramework.NET.UserService.ConsoleWCF
{
    class Program
    {
        static void Main(string[] args)
        {
            var wHost = new System.ServiceModel.Web.WebServiceHost(typeof(UserService), new Uri(ConfigurationManager.AppSettings["baseUrl"]));
            var httpBinding = new WebHttpBinding();
            wHost.AddServiceEndpoint(typeof(IUserService), httpBinding, ConfigurationManager.AppSettings["relativeUrl"]);
